...
def _flash_attention_forward_illava(
    ...
    if attention_mask is not None:
        ...
        if not return_attn_weights:
            ...
            attn_output_unpad, attn_weights, _ = flash_attn_varlen_func(
                query_states,
                key_states,
                value_states,
                cu_seqlens_q=cu_seqlens_q,
    ...
    # Note: the `torch.diff(...)` condition is last to use short-circuit and avoid the cuda synchronization it incurs during inference (query_length == 1 always)
        ...
        if not return_attn_weights:
            ...
            attn_output, attn_weights, _ = flash_attn_func(
                query_states, key_states, value_states, dropout, softmax_scale=softmax_scale, causal=causal, return_attn_probs=True, **flash_kwargs
            )
